...
Takes the week's mistake data as a JSON array, sends it to Claude for analysis,
and renders a clean HTML email using a Jinja2 template.
...
Usage:
    python tools/compose_weekly_report.py \
        --since 2026-05-13 --until 2026-05-19 \
        --mistakes-json .tmp/week_2026-05-13.json \
        --streak 5
...
<div class="footer">
  Generated by your English Learning Agent &mdash; {{ generated_at }}{% if doc_url %}<br>
  <a href="{{ doc_url }}">View Writing Journal →</a>{% endif %}
</div>
...
def get_report_data(mistakes, since, until, streak=0, longest_streak=0):
    ...
    summary = {
        ...
        "total_entries": len({m.get("source_ref") or m.get("page_id", "") for m in mistakes}),
...
def main():
    ...
    parser.add_argument("--mistakes-json", help="JSON array of mistake dicts (default stdin)")
    ...
    days_written = len({m["date"] for m in mistakes if m.get("date")})
    n_entries = len({m.get("source_ref") or m.get("page_id", "") for m in mistakes})
    print(f"Composing report for {len(mistakes)} mistakes across {n_entries} entries...", file=sys.stderr)
    data = get_report_data(mistakes, args.since, until, streak=args.streak, longest_streak=args.longest_streak)

    total_entries = len({m.get("source_ref", m.get("page_id", "")) for m in mistakes if m.get("source_ref") or m.get("page_id")}) or 1
